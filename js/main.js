...
$("#contactForm").validator().on("submit", function(event) {
    if (event.isDefaultPrevented()) {
        formError();
        submitMSG(false, "Por favor, revisa los campos y llénanlos correctamente.");
    } else {
    ...
    if(valid == 1) {
        ...
        ServiceHandler.SendMail($.contactForm,function(response){
            swal({
                ...
                function () {
                    window.location.href = "http://www.cincod.mx/info.html";
                }, function (dismiss) {
                    window.location.href = "http://www.cincod.mx/info.html";
                }
            );
...
$(window).scroll(function() {
    ...
    $('.page-section').each(function(i) {
        if ($(this).position().top <= scrollDistance + 145) {
